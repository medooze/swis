...
Reflector.prototype.reflect = function(mirror,options) 
{
			...
			while(rules && rules.length && i<rules.length) 
			{
				...
				//Check if it is a CSSFont
					...
					if (rules[i].type===CSSRule.STYLE_RULE)
					{
						...
						//Check if uses float
						if (options.refloat && rules[i].style && rules[i].style.float)
						{
							//Get all matching divs
	...
	function init(href,html,userAgent)
	{
		...
		if (agent.mobile_safari || agent.android)
			//Hide scrollbars
			self.options.hoveredScrollBars = true;
		//If it is ipad/iphone
		if (agent.mobile_safari)
			//Fix refloat bug in chrome
			self.options.refloat = true;
						...
						try {
							...
							switch(type)
							{
								case MessageType.HTML:
									...
									//Init
									(message.href,message.html, message.userAgent);
									break;
