							...
							switch(type)
							{
								...
								case MessageType.Attributes:
									...
									if (message.key === "disabled" && target.nodeName === "STYLE")
									{
										...
										//For each one
										for (var i=0;i<childs.length;++i)
											//If it child is a media query itself
											if (childs[i].dataset["swisMediaRuleId"])
												//If it is disabled or parent is disabled
								...
								case MessageType.MediaQueryMatches:
									...
									for (var id in message.matches)
									{
										...
										if (mediarules[id].element === mediarules[id].owner)
										{
											...
											//Set top style disabled
											mediarules[id].element.disabled = disabled;
											//Get childs (if any)
