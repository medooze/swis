...
Observer.prototype.observe = function(exclude,wnd,href)
{
	...
	//IE HACK: IE does not allow to insert a cloned node into a different element
	var clonedDocument;
	var cloneNode = function(node)
	{
		...
		if (!document.documentMode)
			...
			return (clonedDocument=node.cloneNode(false));
		
		//Check if it is a text node
		if (node.nodeName === "#comment")
			//Comment
			cloned = clonedDocument.createComment(node.data);
		else if (node.nodeName === "#text")
			//Text
			cloned = clonedDocument.createTextNode(node.textContent);
		else
			//Element
			cloned = clonedDocument.createElement(node.nodeName);
