...
MessageFactory.prototype.flush = function(useBlob,chunked)
{
	...
	if (useBlob) 
	{
		...
		//Create header
		ByteBuffer header = new ByteBuffer(5);
		//Set the message size
		header.writeVartint32(this.size);
		...
		if (left+head.byteLength<chunkSize)
			//Limit it
			chunkSize = left+head.byteLength;
		...
		for (var i=0;i<this.parts.length;i++)
		{
			//Check if we can append all
			if (offset+this.parts[i].byteLength<=chunkSize)
			{
				//Append
				buffer.set(new Uint8Array(this.parts[i]),offset);
				//Increase size
				offset += this.parts[i].byteLength;
				//Decrease left
				left -= this.parts[i].byteLength; 
			} else {
				//Get what can we push o this chunk
				int len = chunkSize-offset;
				//Put it on this chunk
				buffer.set(new Uint8Array(this.parts[i],0,len),offset);
				//Decrease left
				left -= len;
				...
				data.push(chunk);
				//Append
				buffer.set(new Uint8Array(this.parts[i],len),offset);
				//Increase size
				offset += this.parts[i].byteLength-len;
				//Decrease left
				left -= this.parts[i].byteLength-len; 
